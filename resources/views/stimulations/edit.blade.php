...
<head>
    ...
    <title>Edit Stimulasi - Golden Age</title>
    ...
    <style>
        ...
        .current-image img { max-width: 100%; max-height: 300px; border-radius: 10px; border: 3px solid #e2e8f0; }
    ...
    <div class="container">
        <div class="form-card">
            <div class="form-header">
                <h2><i class="bi bi-pencil-square me-2"></i>Edit Stimulasi</h2>
                <p>Perbarui informasi aktivitas stimulasi</p>
            </div>
            ...
            <form action="{{ route('stimulations.update', $stimulation->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror" 
                               id="title" name="title" value="{{ old('title', $stimulation->title) }}" 
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('category') is-invalid @enderror" 
                            ...
                            <option value="Motorik Kasar" {{ old('category', $stimulation->category) == 'Motorik Kasar' ? 'selected' : '' }}>Motorik Kasar</option>
                            <option value="Motorik Halus" {{ old('category', $stimulation->category) == 'Motorik Halus' ? 'selected' : '' }}>Motorik Halus</option>
                            <option value="Kognitif" {{ old('category', $stimulation->category) == 'Kognitif' ? 'selected' : '' }}>Kognitif</option>
                            <option value="Bahasa" {{ old('category', $stimulation->category) == 'Bahasa' ? 'selected' : '' }}>Bahasa</option>
                            <option value="Sosial-Emosional" {{ old('category', $stimulation->category) == 'Sosial-Emosional' ? 'selected' : '' }}>Sosial-Emosional</option>
                        </select>
                        @error('category')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('age_range') is-invalid @enderror" 
                            ...
                            <option value="0-3 bulan" {{ old('age_range', $stimulation->age_range) == '0-3 bulan' ? 'selected' : '' }}>0-3 bulan</option>
                            <option value="4-6 bulan" {{ old('age_range', $stimulation->age_range) == '4-6 bulan' ? 'selected' : '' }}>4-6 bulan</option>
                            <option value="7-9 bulan" {{ old('age_range', $stimulation->age_range) == '7-9 bulan' ? 'selected' : '' }}>7-9 bulan</option>
                            <option value="10-12 bulan" {{ old('age_range', $stimulation->age_range) == '10-12 bulan' ? 'selected' : '' }}>10-12 bulan</option>
                        </select>
                        @error('age_range')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" class="form-control @error('duration') is-invalid @enderror" 
                               id="duration" name="duration" value="{{ old('duration', $stimulation->duration) }}" 
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" 
                                  ...
                                  placeholder="Jelaskan aktivitas ini secara singkat..." required>{{ old('description', $stimulation->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea class="form-control @error('materials') is-invalid @enderror" 
                                  ...
                                  placeholder="Contoh:&#10;- Mainan warna-warni&#10;- Kain lembut&#10;- Boneka kecil" required>{{ old('materials', $stimulation->materials) }}</textarea>
                        @error('materials')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea class="form-control @error('benefits') is-invalid @enderror" 
                                  ...
                                  placeholder="Contoh:&#10;- Meningkatkan koordinasi mata dan tangan&#10;- Melatih kemampuan motorik halus&#10;- Menguatkan ikatan emosional" required>{{ old('benefits', $stimulation->benefits) }}</textarea>
                        @error('benefits')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        @if($stimulation->image)
                            <div class="current-image">
                                ...
                                <img src="{{ asset('storage/' . $stimulation->image) }}" alt="{{ $stimulation->title }}">
                            </div>
                        @endif
                        ...
                        <div class="image-preview" id="imagePreview">
                            <p class="text-muted small mb-2 mt-3">Preview gambar baru:</p>
                ...
                <div class="d-flex justify-content-between mt-4">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-check-circle me-2"></i>Update Stimulasi
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Apakah Anda yakin ingin menghapus stimulasi <strong>"{{ $stimulation->title }}"</strong>? 
                    Tindakan ini tidak dapat dibatalkan.
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                    <form action="{{ route('stimulations.destroy', $stimulation->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
